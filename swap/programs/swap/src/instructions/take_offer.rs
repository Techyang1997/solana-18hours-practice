...
pub fn withdraw_and_close_vault (ctx: Context<TakeOffer>) -> Result<()> {
    ...
    let accounts = TransferChecked {
        ...
        mint: ctx.accounts.mint_token_a.to_account_info(),
    };
    ...
    transfer_checked(cpi_context,ctx.accounts.vault.amount,ctx.accounts.mint_token_a.decimals)?;
